...
function AnswerPanel({ isAnalyzed, question }) {
  return (
    <section aria-labelledby="answer-heading" className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.04] p-5 sm:p-6">
      <div className="flex items-center gap-2 text-emerald-300">
        ...
        <h2 className="text-xs font-semibold uppercase tracking-[0.16em]" id="answer-heading">{isAnalyzed ? 'Preparation context' : 'Example answer'}</h2>
      </div>
      ...
      <p className="mt-4 font-mono text-[10px] text-slate-600">{isAnalyzed ? 'PROJECT-BASED PREPARATION' : 'MOCK INTERVIEW CONTENT'}</p>
    </section>
  )
}
